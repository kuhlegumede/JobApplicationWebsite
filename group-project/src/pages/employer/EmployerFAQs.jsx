...
export function EmployerFAQs() {
  ...
  const loadFaqs = async () => {
    try {
      setLoading(true);
      setError(null);
      const response = await faqService.getFaqsByCategory("Employers");
      setFaqs(response || []);
    } catch (err) {
              ...
              <div className="card-body p-4 p-md-5">
                ...
                <div className="text-center mb-5">
                  ...
                  <p className="text-muted lead">
                    Find answers to common questions about posting jobs, managing applications, and using NextStep as an employer.
                  </p>
                </div>
                ...
                {/* FAQ Accordion */}
                {!loading && !error && faqs.length > 0 && (
                  <div className="accordion accordion-flush" id="employerFaqAccordion">
                    {faqs.map((faq, index) => (
                      <FAQItem
                        key={faq.faqId}
                        id={`employer-${faq.faqId}`}
